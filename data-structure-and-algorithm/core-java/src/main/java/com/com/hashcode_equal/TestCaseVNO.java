...
public class TestCaseVNO {
	...
	out.printf (" latest: %s, inShops: %s, older: %s%n" ,latest, inShops, older);
	out.println(" latest == inShops: " + (latest == inShops)); // (6)
	out.println(" latest.equals(inShops): " +(latest.equals(inShops))); // (7)
	...
	out.println(" Search key " + searchKey + " has hash code: " +searchKey.hashCode()); // (20)
	out.println(" Map contains search key " + searchKey + ": " +versionStatistics.containsKey(searchKey)); // (21)
	...
	out.println("Binary search in list found key " + searchKey +" at index: " + resultIndex);
